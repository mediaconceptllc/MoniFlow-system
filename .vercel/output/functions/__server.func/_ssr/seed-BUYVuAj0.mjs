...
//#region node_modules/.nitro/vite/services/ssr/assets/seed-BUYVuAj0.js
...
async function nextId(sql, counter, prefix) {
...
async function nextRawId(sql, counter) {
	return `${counter}_${(await sql`
    update counters set value = value + 1 where name = ${counter} returning value
  `)[0]?.value ?? Date.now()}`;
}
...
async function notify(sql, input) {
	if (!input.userId || input.userId === "system") return;
	await sql`
    insert into notifications (id, user_id, title, body, link, severity)
    values (${await nextRawId(sql, "notification")}, ${input.userId}, ${input.title}, ${input.body ?? null}, ${input.link ?? null}, ${input.severity ?? "info"})
  `;
}
async function ensureProfile(userId, hint) {
  ...
  `;
	const existing = await sql`select * from profiles where id = ${userId} limit 1`;
	if (existing[0]) {
	...
	const countRows = await sql`select count(*)::int as n from profiles`;
	const isFirst = Number(countRows[0]?.n ?? 0) === 0;
	const role = isFirst ? "admin" : "requester";
	const display = hint?.name?.trim() || authRows[0]?.name || hint?.email?.split("@")[0] || authRows[0]?.email?.split("@")[0] || (isFirst ? "Workspace Admin" : "Member");
	await sql`
    insert into profiles (id, email, display_name, role)
    values (${userId}, ${hint?.email ?? authRows[0]?.email ?? null}, ${display}, ${role})
    on conflict (id) do nothing
  `;
	return jsonSafe((await sql`select * from profiles where id = ${userId} limit 1`)[0]);
}
function deny(message = "Forbidden") {
	const err = new Error(message);
	err.status = 403;
	throw err;
}
async function sessionMeta(userId) {
	const sql = await getSql();
	return {
...
async function seedIfEmpty(sql) {
	const existing = await sql`select count(*)::int as n from work_items`;
	...
	for (const w of WORK) {
		...
		const created = (/* @__PURE__ */ new Date(Date.now() - (w.hoursAgo ?? 8) * 36e5)).toISOString();
		await sql`
    ...
    `;
      ...
      values (${await nextRawId(sql, "audit")}, ${"work_item"}, ${id}, ${"new"}, ${w.status}, ${w.owner}, ${"Seeded from MoniChat 2026-08-20 board"}, ${created})
    `;
	}
    ...
    values (${await nextRawId(sql, "decision")}, ${unread}, ${"approve"}, ${"P0 remains. Retention risk if unread state stays wrong through September."}, ${"Saruul Bat"})
  `;
	await sql`
    insert into decisions (id, work_item_id, kind, rationale, approver_name)
    values (${await nextRawId(sql, "decision")}, ${bogd}, ${"revise"}, ${"Scope to sandbox QR pay + KYC. Lucky Draw is not a payment dependency — keep it on the roadmap list, not this item."}, ${"Saruul Bat"})
  `;
	await sql`
    insert into comments (id, entity_type, entity_id, author_name, kind, body)
    values (${await nextRawId(sql, "comment")}, ${"work_item"}, ${unread}, ${"Anu Erdene"}, ${"risk"}, ${"Local cache of unread is written before ack from sync API. Samsung OEM battery savers freeze the worker."})
  `;
	await sql`
    insert into comments (id, entity_type, entity_id, author_name, kind, body)
    values (${await nextRawId(sql, "comment")}, ${"work_item"}, ${workIds[3]}, ${"Nomin"}, ${"evidence"}, ${"QA failed: unblock UI updates, incoming still dropped for 6h. Log attached in test run."})
  `;
	await sql`
    insert into test_runs (id, work_item_id, title, result, environment, evidence, tester_name)
    values (${await nextRawId(sql, "test_run")}, ${workIds[3]}, ${"Unblock round-trip on two devices"}, ${"fail"}, ${"Android 14 pair"}, ${"Message from B never arrives at A after unblock. Screenshot of drop in server log line 4412."}, ${"Nomin"})
  `;
  ...
  `;
	...
	for (const t of TICKETS) {
		...
		ticketIds.push(id);
		const created = /* @__PURE__ */ new Date(Date.now() - (t.hoursAgo ?? 8) * 36e5);
		const slaDue = new Date(created.getTime() + (t.slaHours ?? 24) * 36e5);
		const first = t.firstResponse ? new Date(created.getTime() + 15e5).toISOString() : null;
		const resolved = t.status === "resolved" ? new Date(created.getTime() + 216e5).toISOString() : null;
		await sql`
      insert into tickets (
    ...
    values (${await nextRawId(sql, "audit")}, ${voice}, ${unread}, ${"Possible media playback on Xiaomi — engineering to inspect. Kept as helpdesk ticket; engineering owns the linked bug separately."}, ${"system"})
  `;
	await sql`
    insert into ticket_escalations (id, ticket_id, work_item_id, reason, created_by)
    values (${await nextRawId(sql, "audit")}, ${qrTicket}, ${workIds[5]}, ${"Matches P1 QR image import. Ticket stays in helpdesk; work item stays on the engineering board."}, ${"system"})
  `;
	await sql`
    insert into comments (id, entity_type, entity_id, author_name, kind, body)
    values (${await nextRawId(sql, "comment")}, ${"ticket"}, ${ticketIds[0]}, ${"Enkhjin Support"}, ${"answer"}, ${"OTP-г SMS нөөцөөр илгээлээ. Unitel email gateway дээр SPF алдаатай байгаа — инженер рүү шилжүүлэхгүй, auth incident аль хэдийн нээлттэй."})
  `;
	await sql`
    insert into comments (id, entity_type, entity_id, author_name, kind, body)
    values (${await nextRawId(sql, "comment")}, ${"ticket"}, ${gallery}, ${"Enkhjin Support"}, ${"question"}, ${"Галерей нээхэд унаж байна. Samsung A14 + Android 13. Лог илгээнэ үү?"})
  `;
}
//#endregion
export { nextId as a, seedIfEmpty as c, jsonSafe as i, sessionMeta as l, deny as n, nextRawId as o, ensureProfile as r, notify as s, createServerRpc as t, writeAudit as u };
